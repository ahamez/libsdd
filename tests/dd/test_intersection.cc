...
{
  conf::Values val {0};
  intersection_builder<conf, SDD> ops;
...
{
  {
    ...
    sum_builder<conf, SDD> sum_ops_x;
    sum_ops_x.add(ref);
    sum_ops_x.add(x);
    ASSERT_EQ(x, sum(cxt, std::move(sum_ops_x)));

    sum_builder<conf, SDD> sum_ops_y;
    sum_ops_y.add(ref);
    sum_ops_y.add(y);
...
{
  {
    intersection_builder<conf, conf::Values> ops;
  ...
  }
  {
    ...
    conf::Values val2 {2};
    conf::Values ref {2};
    intersection_builder<conf, conf::Values> ops {val0, val1, val2};
...
TEST_F(intersection_test, iterable)
{
  std::vector<SDD> vec { SDD('a', {0,1}, one)
                       ...
                       , SDD('a', {0,1}, one)
                       , SDD('a', {0,1,2}, one)};
  ASSERT_EQ(SDD('a', {0}, one), sdd::dd::intersection<conf>(vec.begin(), vec.end()));
}
...
{
  ASSERT_EQ(SDD('a', {0}, one), sdd::dd::intersection({ SDD('a', {0,1}, one)
                                                      , SDD('a', {0,2}, one)
                                                      , SDD('a', {0,1}, one)
                                                      , SDD('a', {0,1,2}, one)}));
}
