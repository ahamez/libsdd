...
struct expression_post_visitor
{
  ...
  const
  {
    ...
    for (const auto& arc : n)
    {
      ...
      while (gen)
      {
        yield(SDD<C>(gen.get(), local_res->sdd));
        gen();
      }
    }
  }
  ...
  const
  {
    ...
    if (cit == end) // Last level, avoid to propagate a new coroutine.
    {
      for (const auto& arc : n)
      {
        ...
        yield(SDD<C>(arc.valuation(), arc.successor()));
      }
    }
    else // cit != end
    {
      for (const auto& arc : n)
      {
        ...
        while (gen)
        {
          ...
          {
            valuation = eval.evaluate();
            yield(SDD<C>(valuation, gen.get()));
          }
          else
          {
            yield(SDD<C>(arc.valuation(), gen.get()));
          }
          gen();
        }
      }
    ...
    else // target is contained in this hierarchy
    {
      ...
      for (const auto& arc : node)
      {
        ...
        while(gen)
        {
          ...
          operands.add(SDD<C>(/*nested*/ gen.get(), /*successor*/ local_res->sdd));
          gen();
        }
      }
      try
    ...
    if (o.position() == target_)
    {
      ...
      for (const auto& arc : node)
      {
        ...
        while (gen)
        {
          ...
          operands.add(SDD<C>(valuation_, gen.get()));
          gen();
        }
      }
      try
