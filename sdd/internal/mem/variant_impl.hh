...
/// Instanciated for any index generated by the preprocessor that is greater than the number
/// of contained types in the variant, thus avoiding compilation error.
template <typename Visitor, typename T1, typename Storage2, uint8_t Index2, typename... Types2>
typename enable_if<Index2 >= sizeof...(Types2), typename Visitor::result_type>::type
inner_dispatch_binary(const Visitor&, const T1&, const Storage2&, const uint8_t)
noexcept
{
  assert(false);
  __builtin_unreachable();
...
inner_dispatch_binary( const Visitor& v
                     , const T1& x
                     , const Storage2& storage_2, const uint8_t index_2)
{
...
#define BOOST_PP_LOCAL_MACRO(n)                                          \
    case n: {                                                            \
              typedef typename util::nth<Index2 + n, Types2...>::type U; \
...
#include BOOST_PP_LOCAL_ITERATE()
    ...
    // Fallback: loop on the next unroll_size types
    return inner_dispatch_binary<Visitor, T1, Storage2, Index2 + UNROLL_SIZE + 1, Types2...>
...
struct dispatch_binary
{
  ...
  operator()( const Visitor& v
            , const Storage1& storage_1, const Tuple1<Types1...>& tuple1, const uint8_t index_1
            , const Storage2& storage_2, const Tuple2<Types2...>& tuple2, const uint8_t index_2
            )
  {
#ifndef SDD_VARIANT_UNROLL_BINARY_SIZE
#  define UNROLL_SIZE 31
...
#define BOOST_PP_LOCAL_MACRO(n)                                                      \
      case n: {                                                                      \
                ...
                return inner_dispatch_binary<Visitor, T1, Storage2, 0, Types2...>    \
...
#include BOOST_PP_LOCAL_ITERATE()
  ...
  return dispatch_binary<Index1 + UNROLL_SIZE + 1>()
                        ( v
                        , storage_1, tuple1, index_1 - UNROLL_SIZE - 1
                        , storage_2, tuple2, index_2
                        );
#undef UNROLL_SIZE
  }
};
